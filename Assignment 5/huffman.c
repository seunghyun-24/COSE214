...
int encoding( char *codes[], int ch_freq[], FILE *infp, FILE *outfp){
	...
	int a = 0;
	while(!feof(infp)){
		fread(&lazenca, sizeof(char), 1, infp);
		
		for(int j=0; j<strlen(codes[lazenca]); j++){
			a++;
			
			if(codes[lazenca][j] == '1'){
				num <<= 1;
				num|=0x01;
			}
			else num <<= 1;
			...
			if(a==8){
				fwrite(&num, sizeof(unsigned char), 1, outfp);
				byte++;
				a = 0;
				num = 0;
			}
	...
	fwrite(&bit, sizeof(char), 4, outfp);
...
void decoding( tNode *root, FILE *infp, FILE *outfp){
	...
	fread(&try, sizeof(int), 1, infp);
	fseek(infp, 256*sizeof(int), SEEK_SET);

	
	fread(&ling, sizeof(unsigned char), 1, infp);
	int a = 0;
	int numbers = 0;
